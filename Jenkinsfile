pipeline {
	...
	stages {
		...
		stage('Release') {
			steps{
        		sh script: 'mvn clean -s  settings.xml --batch-mode release:clean release:prepare release:branch -DbranchName=webDemo -DupdateBranchVersions=true -DupdateWorkingCopyVersions=false'
    		}
    	}
